...
void eco(int16_t *vectorIn, int16_t *vectorOut, uint16_t bufferSize, uint16_t samplingRate, uint16_t delay_ms) {
	...
	for(; bufferSize > 0; bufferSize--) {
		...
		if(bufferSize >= delay_samples) {
		
			vectorOut[bufferSize - 1] = vectorIn[bufferSize - 1] + vectorIn[bufferSize - 1 - delay_samples] / 2;	
		} else {	
		
			vectorOut[bufferSize - 1] = vectorIn[bufferSize - 1];	
		}	
	}
}
